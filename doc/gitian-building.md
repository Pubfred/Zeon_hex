...
*Setup instructions for a gitian build of ZEON using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ZEON
...
Clone the git repositories for zeon and gitian and then checkout the zeon version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/zeon-crypto/zeon.git
cd zeon
...
Follow the instructions in [doc/release-process.md](release-process.md) in the zeon repository
...
To build ZEON (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the zeon repository.

This may take a long time as it also builds the dependencies needed for each descriptor.
These dependencies will be cached after a succful build to avoid rebuilding them when possible.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zeon/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/zeon-crypto/zeon
...
For example:
```bash
URL=https://github.com/crowning-/zeon.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit zeon=${COMMIT} --url zeon=${URL} ../zeon/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zeon=${COMMIT} --url zeon=${URL} ../zeon/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zeon=${COMMIT} --url zeon=${URL} ../zeon/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zeon-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zeon-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/zeon-build.assert
```
...
[zeon/gitian.sigs](https://github.com/zeon-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
