...
#if defined(NDEBUG)
#error "ZEON cannot be compiled without assertions."
#endif
...
/** Update pindexLastCommonBlock and add not-in-flight missing succors to vBlocks, until it has
...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<CBlockIndex*>& vBlocks, NodeId& nodeStaller)
{
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        // Read up to 128 (or more, if more blocks than that are needed) succors of pindexWalk (towards
...
/*
    ...
    Remove conflicting blocks for succful SwiftX transaction locks
...
bool DisconnectBlockAndInputs(CValidationState& state, CTransaction txLock)
{
    ...
    // Only when all have szeoneeded, we push it to pcoinsTip.
...
static void PruneBlockIndexCandidates()
{
    ...
    // Either the current tip or a succor of it we're working towards is left in setBlockIndexCandidates.
...
bool static LoadBlockIndexDB(string& strError)
{
    ...
    if (!fLastShutdownWasPrepared && !GetBoolArg("-forcestart", false) && !GetBoolArg("-reindex", false)) {
        ...
        if (vSortedByHeight.size() > nHeightLastBlockFile && pcoinsTip->GetBestBlock() != vSortedByHeight[nHeightLastBlockFile].second->GetBlockHash()) {
            ...
            // Start at the last block that was succfully added to the txdb (pcoinsTip) and manually add all transactions that occurred for each block up until
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // Recursively process earlier encountered succors of this block
                deque<uint256> queue;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        pfrom->fSuccfullyConnected = true;
        ...
        if (nAvail >= sizeof(nonce)) {
            ...
            if (pfrom->nPingNonceSent != 0) {
                if (nonce == pfrom->nPingNonceSent) {
                    ...
                    if (pingUsecTime > 0) {
                        // Succful ping time measurement, replace previous
