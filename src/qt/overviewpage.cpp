...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::ZEON)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_ZEON4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_ZEON, SIGNAL(clicked()), this, SLOT(openMyAddresses()));
       
    }

    // update the display unit, to not use the default ("ZEON")
    updateDisplayUnit();
}
...
void OverviewPage::updatBlockChainInfo()
{
    ...
    ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardZEON));
    //ui->label_ZEONSupply_value->setText(QString::number(ZEONSupply));
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeZeonAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeZeonAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeZeonAmount = strAnonymizeZeonAmount.remove(strAnonymizeZeonAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZeonAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeZeonAmount * COIN) nMaxToAnonymize = nAnonymizeZeonAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeZeonAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeZeonAmount));
        strAnonymizeZeonAmount = strAnonymizeZeonAmount.remove(strAnonymizeZeonAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZeonAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::obfuscationReset()
{
    ...
    QMessageBox::warning(this, tr("Obfuscation"),
        tr("Obfuscation was succfully reset."),
