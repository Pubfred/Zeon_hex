...
void MultisigDialog::on_createButton_clicked()
{
    ...
    try{
        ...
        if(validInput){
            ...
            QString status(strprintf("Transaction has succfully created with a fee of %s.\n"
...
bool MultisigDialog::createMultisigTransaction(vector<CTxIn> vUserIn, vector<CTxOut> vUserOut, string& feeStringRet, string& errorRet)
{
    try{
        ...
        if(totalIn < totalOut){
            throw runtime_error("Not enough ZEON provided as input to complete transaction (including fee).");
        }
        ...
        if(tx.vout.at(changeIndex).nValue > fee){
           ...
           feeStringRet = strprintf("%d",((double)fee)/COIN).c_str();
            ...
            throw runtime_error("Not enough ZEON provided to cover fee");
        }
...
void MultisigDialog::commitMultisigTx()
{
    ...
    try{
        ...
        RelayTransaction(tx);
#endif
        //disable commit if succfully committed
        ui->commitButton->setEnabled(false);
        ui->signButtonStatus->setText(strprintf("Transaction has been succfully published with transaction ID:\n %s", tx.GetHash().GetHex()).c_str());
    }catch(const runtime_error& e){
