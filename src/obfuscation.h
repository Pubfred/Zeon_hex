...
#define POOL_STATUS_SZEONESS 8              // succ
...
public:
     ...
     *     3) we signed the message succfully, and
     *     4) we verified the message succfully
     */
    bool Sign();
...
{
public:
    /// Is the inputs associated with this public key? (and there is deposit ZEON amount - checking if valid masternode)
    bool IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey);
    /// Set the private/public key values, returns true if succful
    bool GetKeysFromSecret(std::string strSecret, CKey& keyRet, CPubKey& pubkeyRet);
    /// Set the private/public key values, returns true if succful
    bool SetKey(std::string strSecret, std::string& errorMessage, CKey& key, CPubKey& pubkey);
    /// Sign the message, returns true if succful
    bool SignMessage(std::string strMessage, std::string& errorMessage, std::vector<unsigned char>& vchSig, CKey key);
    /// Verify the message, returns true if szeoncessful
...
public:
    ...
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SZEONESS)) {
            LogPrint("obfuscation", "CObfuscationPool::UpdateState() - Can't set state to ERROR or SZEONESS as a Masternode. \n");
            return;
        }
    ...
    /// Get the denominations for a specific amount of ZEON.
