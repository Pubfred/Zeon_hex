...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //Obfuscation Accept Into Pool
        ...
        BOOST_FOREACH (const CTxIn item, sigs) {
            if (AddScriptSig(item)) succ = true;
...
//
// Unlock coins after Obfuscation fails or szeoneeds
//
...
std::string CObfuscationPool::GetStatus()
{
    ...
    if (chainActive.Tip()->nHeight - cachedLastSucc < minBlockSpacing || !masternodeSync.IsBlockchainSynced()) {
...
void CObfuscationPool::Check()
{
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SZEONESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
...
void CObfuscationPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        // Tell the clients it was succful
        RelayCompletedTransaction(sessionID, false, MSG_SZEONESS);
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat ZEON and make it unusable. To
                stop these kinds of attacks 1 in 10 succful transactions are charged. This
                adds up to a cost of 0.001 ZEON per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::CheckTimeout()
{
    ...
    if (!fMasterNode) {
        ...
        case POOL_STATUS_ERROR:
            ...
            break;
        case POOL_STATUS_SZEONESS:
            LogPrint("obfuscation", "CObfuscationPool::CheckTimeout() -- Pool succ -- Running Check()\n");
            Check();
            break;
        }
    }
...
bool CObfuscationPool::SignatureValid(const CScript& newSig, const CTxIn& newVin)
{
    ...
    LogPrint("obfuscation", "CObfuscationPool::SignatureValid() - Signing - Succfully validated input\n");
    return true;
}
...
{
    if (fMasterNode) return false;
    if (state == POOL_STATUS_ERROR || state == POOL_STATUS_SZEONESS) return false;
...
// Obfuscation transaction was completed (failed or succful)
void CObfuscationPool::CompletedTransaction(bool error, int errorID)
{
    ...
    if (error) {
        ...
        LogPrintf("CompletedTransaction -- succ \n");
        UpdateState(POOL_STATUS_SZEONESS);

        UnlockCoins();
        SetNull();

        // To avoid race conditions, we'll only let DS run once per block
        cachedLastSucc = chainActive.Tip()->nHeight;
    }
    lastMessage = GetMessageByID(errorID);
}
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (chainActive.Tip()->nHeight - cachedLastSucc < minBlockSpacing) {
        LogPrintf("CObfuscationPool::DoAutomaticDenominating - Last succful Obfuscation action was too recent\n");
        strAutoDenomResult = _("Last succful Obfuscation action was too recent.");
        return false;
    }
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeZeonAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
bool CObfuscationPool::SendRandomPaymentToSelf()
{
    ...
    bool succ = pwalletMain->CreateTransaction(vecSend, wtx, reservekey, nFeeRet, strFail, coinControl, ONLY_DENOMINATED);
    if (!succ) {
    ...
    LogPrintf("SendRandomPaymentToSelf Succ: tx %s\n", wtx.GetHash().GetHex());
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    bool succ = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
    ...
    if (!succ) {
        ...
        succ = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
    ...
    // use the same cachedLastSucc as for DS mixinx to prevent race
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100ZEON+1 ( bit on if present )
    // bit 1 - 10ZEON+1
    // bit 2 - 1ZEON+1
    // bit 3 - .1ZEON+1
    // bit 3 - non-denom
...
std::string CObfuscationPool::GetMessageByID(int messageID)
{
    ...
    case MSG_SZEONESS:
        return _("Transaction created succfully.");
    case MSG_ENTRIES_ADDED:
        return _("Your entries added succfully.");
    case MSG_NOERR:
    default:
        return "";
    }
