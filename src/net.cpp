...
{
    if (fListen && pnode->fSuccfullyConnected) {
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (ZEON users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "ZEON " + FormatFullVersion();
        ...
        try {
            while (true) {
                ...
                if (r != UPNPCOMMAND_SZEONESS)
                    ...
                    LogPrintf("UPnP Port Mapping succful.\n");
                ;
...
void ThreadOpenAddedConnections()
{
    ...
    for (unsigned int i = 0; true; i++) {
        ...
        // Attempt to connect to each IP for each addnode entry until at least one is succful per addnode entry
...
// if succful, this moves the passed grant to the constructed node
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. ZEON Core is probably already running."), addrBind.ToString());
        else
...
{
    if (!fSuccfullyConnected) return; // Don't fuzz initial handshake
