...
bool CAddrInfo::IsTerrible(int64_t nNow) const
{
    ...
    if (nLastSucc == 0 && nAttempts >= ADDRMAN_RETRIES) // tried N times and never a succ
        return true;

    if (nNow - nLastSucc > ADDRMAN_MIN_FAIL_DAYS * 24 * 60 * 60 && nAttempts >= ADDRMAN_MAX_FAILURES) // N succive failures in the last week
        return true;
